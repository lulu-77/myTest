$(function() {
	...
	$(".all").click(function() {
		...
		if($('.all>span').hasClass('normal')){
			...
			$('.all>span>img').attr('src','../images/cart/product_true2.png');
			$(".Each>span").each(function() {
				$(this).addClass('true').removeClass('normal');
				$(this).children('img').attr('src','../images/cart/product_true2.png');
			})
	...
	$('.Each>span').click(function(){
		...
		if($(this).hasClass('normal')){
			$(this).addClass('true').removeClass('normal');
			$(this).children('img').attr('src','../images/cart/product_true2.png');
			var amou=parseFloat($('.total').text());
			amou++;
			$('.total').text(amou);
			$('.totalOne').text(amou);
			amountadd();
			...
			$('.total').text(amou);
			$('.totalOne').text(amou);
			var newamo=parseFloat($('.susum').text())-parseFloat($(this).parent().parent().siblings('.totle').children('.totle_information').text());
...
function adddel(){
	...
	$(".add").each(function() {
		$(this).click(function() {
			...
			$multi = (parseFloat(vall).toFixed(2) * parseFloat($(this).parent().prev().children().eq(1).children().eq(1).text()));
			console.log($multi);
	...
	$(".reduc").each(function() {
		$(this).click(function() {
			...
			$(this).next().val(vall1);
			$multi1 = parseFloat(vall1) * parseFloat($(this).parent().prev().children().eq(1).children().eq(1).text());
